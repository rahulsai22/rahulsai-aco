...
def run_aco(space, iters, colony, alpha, beta, del_tau, rho):
...
# Read CSV data file
post_off_data = pandas.read_csv(r'miniproj(1).csv')
# Create array of latitude and longitude


space = np.array([post_off_data.values[:, 4], post_off_data.values[:, 5]], dtype=np.float_)
# Rearrange data
